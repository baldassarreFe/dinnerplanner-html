var OverviewDishesViewController = function(view, model) {
	 var updateSelectedGrid = function(sel, num) {
		 view.overviewCardGrid.empty();
		  sel.forEach(d => {
			var cardHtml = `
				<div id="dish-${d.id}" class="col-lg-3 col-md-4 col-xs-6 dish-card col-centered">
					<img class="img-responsive" src="${d.image}" alt="" width="140" height="140">
					<p class="dish-title"><strong>${d.name}</strong></p>
					<p class="dish-description">${shortDescription(d.description)}</p>
